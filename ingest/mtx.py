...
class MTXIngestor(GeneExpression):
    ...
    def create_data_array(self, gene: str, array_type: str, values: List[str], linear_data_type: str, id) -> List:
        data_arrays = []
        for model in self.set_data_array(gene, array_type, values, linear_data_type, id):
    ...
    def transform(self):
        ...
        data_arrays.extend(
            self.create_data_array(f'{self.cluster_name} Cells', 'cells', self.cells, 'Study', self.study_file_id)
        )
        for row in self.mtx_file:
            ...
            if current_idx != last_idx:
                if MTXIngestor.is_sorted(current_idx, visited_expression_idx):
                    ...
                    gene_model = self.create_gene_model(gene,
                                                        self.study_file_id,
                                                        self.study_id,
                                                        gene_id, model_id)
