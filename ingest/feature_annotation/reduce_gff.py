"""Convert Ensembl gene annotation GFF files to minimal TSVs

SCP uses cached gene data for search-by-gene-name for expression / feature /
variable data that:
* Has only Ensembl gene ID
* Potentially, has only genomic ranges
"""
...
def get_gff_url(organism):
    """Get URL to GFF file
    E.g. https://ftp.ensembl.org/pub/release-111/gff3/homo_sapiens/Homo_sapiens.GRCh38.102.gff3.gz
    """
    release = "111"
...
def detect_prefix(id):
    ...
    Feature IDs in a given Ensembl GFF file have a constant "prefix".
...
def parse_gene(gff_row):
    ...
    if feat_type not in ref_feature_types:
        return None
...
class GeneCache():
    """Convert Ensembl GFF files to minimal TSVs for SCP gene caches
    """
    ...
    def write(self, genes, organism, prefix, gff_url):
        ...
        columns = ["# chr", "start", "length", "slim_id", "symbol"]
        column_headers = "\t".join(columns)

        file_headers = [
            f"## SCP gene cache for {organism}",
    ...
    def populate_by_org(self, organism):
        ...
        self.write(slim_genes, organism, prefix, gff_url)
