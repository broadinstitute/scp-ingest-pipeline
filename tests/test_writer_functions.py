...
class TestWriterFunctions(unittest.TestCase):
    def test_is_gz_file(self):
        self.assertTrue(
            is_gz_file('data/mtx/unsorted_matrix.mtx.gz')
        )
        self.assertFalse(
            is_gz_file('data/mtx/matrix.mtx')
        )

    def test_round_exp(self):
        self.assertEqual(
            1.123, round_exp(1.123456789, 3)
        )
    ...
    def test_encode_cluster_name(self):
        ...
        self.assertEqual(
            expected_name, encode_cluster_name(name)
        )
        name_with_pos = 'CD4+ T cells'
        expected_name_pos = 'CD4pos_T_cells'
        self.assertEqual(
            expected_name_pos, encode_cluster_name(name_with_pos)
        )
    ...
    def test_open_file(self):
        ...
        self.assertEqual(
            expected_line, file_io.readline()
        )
    ...
    def test_extract_sparse_line(self):
        ...
        self.assertTrue(
            expected_data, extract_sparse_line(line)
        )

    def test_process_dense_line(self):
        line = "\"Gad1\"\t0\t0\t1.23423\t0\t0\n"
        cluster_cells = [
            'CTAACTTGTTCCATGA-1', 'CTCCTAGGTCTCATCC-1', 'CTCGGAGTCGTAGGAG-1',
            'CTGAAACAGGGAAACA-1', 'GACTACAGTAACGCGA-1'
        ]
        matrix_cells = [
            'CTCGGAGTCGTAGGAG-1', 'CTGAAACAGGGAAACA-1', 'CTCCTAGGTCTCATCC-1',
            'GACTACAGTAACGCGA-1', 'CTAACTTGTTCCATGA-1'
        ]
    ...
    def test_filter_expression_for_cluster(self):
        ...
        matrix_cells = [
            'GACTACAGTAACGCGA-1', 'CTGAAACAGGGAAACA-1', 'GCATACAGTACCGTTA-1',
            'GCGAGAACAAGAGGCT-1', 'GCTTCCACAGCTGTAT-1'
        ]
        ...
        filtered_data = list(filter_expression_for_cluster(cluster_cells, matrix_cells, expression))
        self.assertEqual(
