...
class TestExpressionWriter(unittest.TestCase):
    ...
    def seed_test_gene_entries(data_dir):
        for file in os.listdir('data/expression_writer/gene_entries'):
            copy(f"data/expression_writer/gene_entries/{file}", f"{data_dir}/gene_entries")
    ...
    def test_divide_sparse_matrix(self):
        cluster_name = encode_cluster_name(f"{self.TEST_PREFIX}sparse_divide_{uuid.uuid4()}")
        exp_writer = self.setup_sparse_exp_writer(cluster_name)
    ...
    def test_read_sparse_matrix_slice(self):
        cluster_name = encode_cluster_name(f"{self.TEST_PREFIX}sparse_slice_{uuid.uuid4()}")
        indexes = [136, 219]
        exp_writer = self.setup_sparse_exp_writer(cluster_name)
        make_data_dir(cluster_name)
        genes = load_entities_as_list(open(exp_writer.gene_file))
        ...
        expected_data = open('data/expression_writer/gene_entries/OXCT2__entries.txt').read()
    ...
    def test_process_sparse_data_fragments(self):
        cluster_name = encode_cluster_name(f"{self.TEST_PREFIX}sparse_fragments_{uuid.uuid4()}")
        exp_writer = self.setup_sparse_exp_writer(cluster_name)
        make_data_dir(cluster_name)
    ...
    def test_write_empty_sparse_genes(self):
        cluster_name = encode_cluster_name(f"{self.TEST_PREFIX}sparse_empty_genes_{uuid.uuid4()}")
        exp_writer = self.setup_sparse_exp_writer(cluster_name)
        make_data_dir(cluster_name)
        self.seed_test_gene_entries(cluster_name)
        num_cells = 25
    ...
    def test_read_dense_matrix_slice(self):
        cluster_name = encode_cluster_name(f"{self.TEST_PREFIX}dense_slice_{uuid.uuid4()}")
        exp_writer = self.setup_dense_exp_writer(cluster_name)
        make_data_dir(cluster_name)
        indexes = [161, 264]
